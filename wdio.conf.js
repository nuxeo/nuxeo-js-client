...
// Static server mounts only the directories needed by browser-tests.html.
// Avoids exposing the entire project root (dotfiles, configs, etc.) through
// Sauce Connect to remote browser VMs.
const staticServerConfig = {
  folders: [
    { mount: `/${distDir}`, path: `./${distDir}` },
    { mount: `/${buildDir}`, path: `./${buildDir}` },
    { mount: '/test', path: './test' },
    { mount: '/node_modules/mocha', path: './node_modules/mocha' },
  ],
  port: 8888,
};
