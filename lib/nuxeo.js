...
class Nuxeo extends Base {
  ...
  connect(opts = {}) {
    ...
    return this.http(finalOptions)
      ...
      .then((res) => this.users({ enrichers: { user: ['userprofile'] } }).fetch(res.id || res.username))
      .then((user) => {
        this.user = user;
        this.connected = true;
        return this;
      ...
      doFetch(options.url, fetchOptions)
        .then((res) => {
          ...
          if (!(/^2/.test(`${res.status}`))) {
            // Native fetch in Node.js (undici) leaves statusText empty; browsers populate it.
            const error = new Error(res.statusText || `${res.status}`);
            error.response = res;
            return reject(error);
          }
  ...
  _computeFetchOptions(opts) {
    ...
    if (options.queryParams && Object.keys(options.queryParams).length > 0) {
      ...
      options.url += qs.stringify(options.queryParams, { arrayFormat: 'repeat' });
    }
    return options;
  }
