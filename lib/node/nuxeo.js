...
var Client = function(options) {
  ...
  var RestService = rest.service(function() {
    if (self._authentication.method === 'basic') {
      this.defaults.username = self._authentication.username;
      this.defaults.password = self._authentication.password;
    }
  }, {
...
Client.prototype._initAuthentication = function() {
  switch (this._authentication.method) {
    case 'proxy':
      var proxyAuthHeaderName = this._authentication.proxyAuthHeaderName || authentication.proxy.DEFAULT_HEADER_NAME;
      this._headers[proxyAuthHeaderName] = this._authentication.username;
      break;

    case 'portal':
      var date = new Date();
      var randomData = random.integer();

      var clearToken = date.getTime() + authentication.portal.TOKEN_SEPARATOR
        + random + authentication.portal.TOKEN_SEPARATOR
        + this._authentication.secret + authentication.portal.TOKEN_SEPARATOR
        + this._authentication.username;

      var base64hashedToken = crypto.createHash('md5')
        .update(clearToken, 'utf8').digest('base64');
      this._headers[authentication.portal.headerNames.RANDOM] = randomData;
      this._headers[authentication.portal.headerNames.TIMESTAMP] = date.getTime();
      this._headers[authentication.portal.headerNames.TOKEN] = base64hashedToken;
      this._headers[authentication.portal.headerNames.USER] = this._authentication.username;
      break;
  }
};
...
Client.prototype.connect = function(callback) {
  ...
  if (this._authentication.method === 'basic') {
    options.username = this._authentication.username;
    options.password = this._authentication.password;
  }
  ...
  rest.post(join(this._automationURL, 'login'), options)
    .on('complete', function(data) {
      if (data instanceof Error) {
        ...
        try {
          if (data['entity-type'] === 'login' && data['username'] === self._authentication.username) {
            self.connected = true;
            if (callback) {
              callback(null, self)
            }
