...
var Client = function(options) {
  ...
  this._methodAuth = options.methodAuth;
  this._username = options.username;
  this._password = options.password;
  this._secret = options.secret || 'nuxeo5secret';
  this._proxyHeader = options.proxyHeader || 'Auth-User';
...
Client.prototype._setAuth = function() {
  if(this._methodAuth == 'proxy') {
    this.proxyAuth(this._proxyHeader, this._username);
  } else if (this._methodAuth == 'portal') {
    this.portalAuth(this._secret, this._username);
  }
}
...
Client.prototype.basicAuth = function(username, password) {
    this._methodAuth = 'basic';
    this._username = username;
    this._password = password;
    this._cleanAuthenticationHeaders();
}

Client.prototype.proxyAuth = function(headerName, username) {
    this._methodAuth = 'proxy';
    this._proxyHeader = headerName;
    this._username = username;
    this._password = '';
    this._cleanAuthenticationHeaders();
    this.header(this._proxyHeader, this._username);
}

Client.prototype.portalAuth = function(secret, username) {
    this._methodAuth = 'portal';
    this._secret = secret;
    this._username = username;
    this._cleanAuthenticationHeaders();
    this._renewPortalToken();
}

Client.prototype._renewPortalToken = function() {
    // Compute hashed token
    var date = new Date();
    var random = format(crypto.randomBytes(3), 'dec');
    var clearToken = date.getTime()
        + ':' + random
        + ':' + this._secret
        + ':' + this._username;
    var clearTokenBytes = new Buffer(clearToken, 'utf8');
    var md5sum = crypto.createHash('md5');
    var base64hashedToken = md5sum.update( clearTokenBytes ).digest('base64');
    // Update headers for portal authentication
    this.header(PORTAL_AUTH_HEADERS.random, random);
    this.header(PORTAL_AUTH_HEADERS.timestamp, date.getTime());
    this.header(PORTAL_AUTH_HEADERS.token, base64hashedToken);
    this.header(PORTAL_AUTH_HEADERS.user, this._username);
}

// Remove authentication informations from headers
Client.prototype._cleanAuthenticationHeaders = function () {
    var headersToRemove = _.union([this._proxyHeader], _.values(PORTAL_AUTH_HEADERS));
    this._headers = _.omit(this._headers, headersToRemove);
}
...
var Document = function(options) {
  ...
  if (typeof data === 'string') {
    ...
    path = join(path, '@blob', 'file:content');
    var request = options.client.request(path);
    this.blobUrl = join(request._url, request._path);
};
...
Document.prototype.create = function(data, callback) {
  ...
  Document.prototype.copy = function(data, callback) {
    ...
    operation.timeout(this._timeout).schemas(this._schemas).headers(this._headers)
      ...
      .input(this.uid).params({ target: data.target });
    operation.execute({ data: data }, function(error, data, response) {
...
Uploader.prototype.uploadFiles = function() {
  ...
  while (this._uploadStack.length > 0) {
    ...
    var file = rest.file(fileobject.path, utf8.encode(fileobject.filename), fileobject.fileSize, fileobject.encoding, fileobject.contentType);
    ...
    headers['X-File-Name'] = utf8.encode(file.filename);
